...
from pgs_recon.openmvg import compute_features, compute_matches, init_sfm_generic, mvg_colorize_sfm, \
    mvg_compute_known, mvg_sfm, mvg_to_mvs
from pgs_recon.openmvs import mvs_densify, mvs_reconstruct, mvs_refine, mvs_texture
...
def main():
    ...
    # Hidden opts
    parser.add_argument('--path', type=str, default='/usr/local/', help=configargparse.SUPPRESS)
    ...
    opts_matcher.add_argument('--matching-ratio', type=float, default=None, help='Nearest-Neighbor distance ratio')
    ...
    # MVG hidden opts
    opts_mvg.add_argument('--sfm-ba', action='store_true', help=configargparse.SUPPRESS)
    opts_mvg.add_argument('--robust-ba', action='store_true', help=configargparse.SUPPRESS)
    opts_mvg.add_argument('--mvg-initializer', choices=['EXISTING_POSE', 'MAX_PAIR', 'AUTO_PAIR', 'STELLAR'],
    ...
    config = paths['output'] / (datetime_str + '_' + args.name + '_recon_config.txt')
    ...
    compute_matches(paths, method=args.matching_method, model=args.matching_geometric_model, ratio=args.matching_ratio,
    ...
    if args.mvg_recon_method == 'direct':
        ...
        sfm_key = mvg_sfm(paths, sfm_key='sfm', method=args.mvg_recon_method, use_priors=args.mvg_priors,
                          refine_intrinsics=args.mvg_refine_intrinsics, initializer=args.mvg_initializer,
                          metadata=metadata)
